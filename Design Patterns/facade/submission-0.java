...
class DriveThruFacade {
    ...
    public Food takeOrder(String orderContents, boolean takeOut) {
        Order customerOrder = cashier.takeOrder(orderContents, takeOut);
        Food customerFood = chef.prepareFood(customerOrder);
        if (customerOrder.isTakeOut()) {
            return kitchenStaff.packageOrder(customerFood);
        }
        return customerFood;
